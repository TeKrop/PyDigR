...
if (argc is 2 or argc is 4) and os.path.isfile(sys.argv[1]):
	...
	img = cv.LoadImageM(sys.argv[1],3)
	
	# On effectue un seuil adaptatif avec une image floutée
	# Afin d'éliminer le maximum de bruit
	gray = cv2.cvtColor(np.asarray(img),cv2.COLOR_BGR2GRAY)
	blur = cv2.GaussianBlur(gray,(41,41),0)
	img_seuil = cv2.adaptiveThreshold(blur, 255, 1, cv2.THRESH_BINARY_INV, 11, 2)

	# On reconvertit dans le format compatible avec la première
	# Version de OpenCV
	img_seuil = cv.fromarray(img_seuil)
